...
erDiagram
  ...
  rss_feeds {
    ...
    string url "NOT NULL"
    string description "NULL allowed"
    string category "NULL allowed"
    datetime last_fetched "NULL allowed"
    datetime created_at "DEFAULT CURRENT_TIMESTAMP"
    datetime updated_at "DEFAULT CURRENT_TIMESTAMP"
    integer user_id FK "FOREIGN KEY REFERENCES users(id) ON DELETE CASCADE"
    constraint unique_feed "UNIQUE(user_id, url)"
    index idx_user "INDEX(user_id)"
    index idx_url_user "INDEX(url, user_id)"
  }

  favorites {
    integer id PK "PRIMARY KEY AUTOINCREMENT"
    integer user_id FK "FOREIGN KEY REFERENCES users(id) ON DELETE CASCADE"
    integer feed_id FK "FOREIGN KEY REFERENCES rss_feeds(id) ON DELETE CASCADE"
    datetime created_at "DEFAULT CURRENT_TIMESTAMP"
    constraint unique_favorite "UNIQUE(user_id, feed_id)"
    index idx_user_feed "INDEX(user_id, feed_id)"
  }

  articles {
    string link PK "PRIMARY KEY"
    integer feed_id FK "FOREIGN KEY REFERENCES rss_feeds(id) ON DELETE CASCADE"
    string title "NOT NULL"
    string description "NULL allowed"
    datetime pub_date "NULL allowed"
    string author "NULL allowed"
    string content "NULL allowed"
    string guid "NULL allowed"
    datetime created_at "DEFAULT CURRENT_TIMESTAMP"
    index idx_feed_date "INDEX(feed_id, pub_date)"
    index idx_guid "INDEX(guid)"
  }

  users ||--o{ rss_feeds : owns
  users ||--o{ favorites : has
  rss_feeds ||--o{ favorites : bookmarked_in
  rss_feeds ||--o{ articles : contains
```
